...
const UserProfile = () => {
  ...
  const handleSelect = async () => {
    const data = await fetch(
      `http://localhost:5000/project/Select/${card[index]._id}`,
      {
        method: "PUT",
        headers: {
  ...
  const handleReject = async () => {
    const data = await fetch(
      `http://localhost:5000/project/Reject/${card[index]._id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleShortlist = async () => {
    const data = await fetch(
      `http://localhost:5000/project/Shortlist/${card[index]._id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleSchedule = async () => {
    const data = await fetch(
      `http://localhost:5000/project/Schedule/${card[index]._id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
            ...
            <div className="right_pfp col-lg-9 col-md-9 col-12">
              <div className="shadow child_user mx-2">
                <div className="p-4 pb-0">
                  ...
                  <div className="h-100">
                    {active === "details" && (
                      <Details Data={userData.basicInfo} />
                    )}
                    {active === "talent" && <Talents Data={userData.talent} />}
                    {active === "bio" && (
                      <BioExperience Data={userData.portfolio} />
                    )}
                    {active === "education" && <Education Data={userData} />}
                    {active === "role" && <UserRole Data={userData} />}
                  </div>
                ...
                {d !== 0 ? (
                  <>
                    ...
                    />
                    <SubViewProfile
                      display={card?.length == index + 1 ? "none" : ""}
                      index={index}
                      card={card}
                      msg={`Next`}
                    />
